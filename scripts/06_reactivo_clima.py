            ...
            with requests.get(url, headers=headers, stream=True, timeout=None) as r:
                for line in r.iter_lines():
                    ...
                    try:
                        ...
                        if item != "AC_CurrentTemp":
                            continue
                        payload = json.loads(data["payload"])
                        valor = payload.get("value")
                        try:
                            temperatura = float(str(valor).split(" ")[0])
                        except ValueError:
                            continue
...
def main():
    print("Sistema reactivo de climatización (escribe 'salir' para terminar)\n")
    print("Mueve el slider AC_CurrentTemp en OpenHAB para activar la lógica reactiva.\n")
